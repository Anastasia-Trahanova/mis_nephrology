...
from app import patient_list_queries
from app.patient_list_queries import PatientListFilters, describe_filters
from app.routers import patient_lists


ROOT = Path(__file__).resolve().parents[2]


def test_patient_list_filters_defaults_and_comma_decimal():
    filters = PatientListFilters.from_mapping({"indicator": "potassium", "value_from": "5,5"})
...
def test_egfr_category_boundaries_are_correct():
    assert patient_list_queries.EGFR_CATEGORIES["С3а"]["minimum"] == Decimal("45")
    assert patient_list_queries.EGFR_CATEGORIES["С3б"]["minimum"] == Decimal("30")
    assert patient_list_queries.EGFR_CATEGORIES["С4"]["minimum"] == Decimal("15")
    assert patient_list_queries.EGFR_CATEGORIES["С5"]["maximum"] == Decimal("15")


def test_manual_range_is_normalized():
    filters = PatientListFilters.from_mapping(
        {
            "indicator": "egfr",
            "mode": "manual",
            "operator": "between",
...
def test_sql_uses_one_ranked_query_and_existing_tables():
    filters = PatientListFilters.from_mapping({"indicator": "hemoglobin"})
    sql = patient_list_queries._patient_list_sql(filters)
...
def test_template_has_dynamic_controls_and_requested_columns():
    template = (ROOT / "app" / "templates" / "patient_lists.html").read_text(encoding="utf-8")
    ...
    assert 'data-registry-dynamics' in template
    assert 'id="registryDynamicsModal"' in template
    assert 'js/patient_lists_dynamics.js' in template


def test_navigation_is_available_to_doctor_and_admin():
    base = (ROOT / "app" / "templates" / "base.html").read_text(encoding="utf-8")
    assert "current_role == 'admin' or current_role in clinical_roles" in base
    assert 'href="/patient-lists"' in base
    assert "Списки пациентов" in base
    assert "Журнал аудита" in base


def test_doctor_can_open_patient_lists(monkeypatch):
    monkeypatch.setattr(
        patient_lists,
        "get_patient_list",
        lambda filters: {
    ...
    app.include_router(patient_lists.router)
    ...
    with TestClient(app) as client:
        ...
        response = client.get("/patient-lists")
...
def test_non_medical_role_is_rejected():
    ...
    with pytest.raises(Exception) as error:
        patient_lists.require_patient_lists_access(SessionRequest())
    assert getattr(error.value, "status_code", None) == 403


def test_csv_contains_first_and_latest_values(monkeypatch):
    monkeypatch.setattr(
        patient_list_queries,
        "get_patient_list",
        lambda filters: {
    ...
    content, filename = patient_list_queries.build_patient_list_csv(PatientListFilters())
...
def test_patient_lists_page_renders_patient_history(monkeypatch):
    monkeypatch.setattr(
        patient_lists,
        "get_patient_list",
        lambda filters: {
...
def test_patient_lists_export_route_is_available_to_doctor(monkeypatch):
    monkeypatch.setattr(
        patient_lists,
    ...
    with TestClient(app) as client:
        client.get("/set-doctor")
        response = client.get("/patient-lists/export.csv")
...
def test_each_indicator_uses_whitelisted_source(indicator, table_name, column_name):
    filters = PatientListFilters.from_mapping({"indicator": indicator})
    sql = patient_list_queries._patient_list_sql(filters)
...
def test_csv_escapes_formula_like_text(monkeypatch):
    ...
    content, _ = patient_list_queries.build_patient_list_csv(PatientListFilters())
    assert "'=HYPERLINK" in content
    assert "'@doctor" in content


def test_patient_lists_hotkeys_cover_page_actions():
    script = (ROOT / "app" / "static" / "js" / "patient_lists_dynamics.js").read_text(encoding="utf-8")
    base = (ROOT / "app" / "templates" / "base.html").read_text(encoding="utf-8")
    ...
    assert '`/patient-lists/patient/${encodeURIComponent(patientId)}/dynamics?' in script
    assert '/ckd-registry/patient/' not in script
    assert "is_patient_lists_page" in base
